...
class AirMobiSim(airmobisim_pb2_grpc.AirMobiSimServicer):
    ...
    def InsertUAV(self, request, context):
        ...
        self.simulation_obj._managedNodes.append(Uav(request.id, [Point(request.coordinates[0].x, request.coordinates[0].y, request.coordinates[0].z), Point(request.coordinates[1].x, request.coordinates[1].y, request.coordinates[1].z)], speed=request.speed, angle=request.angle, model_selection=2))
        return struct_pb2.Value()
